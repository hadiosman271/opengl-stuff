...
// this creates vertex data for a cube
// format: vec3 position, vec3 normal, vec2 texture coordinate
float *box(unsigned *size) {
	*size = sizeof(float [288]);
	float *vert = malloc(*size);

	vec3 pt[4] = { [0][X] = 0.5f };
	pt[0][Z] = pt[0][Y] = pt[0][X];
	...
	for (int i = 0, one = 1; i < 6; i++) {
		...
		i % 3 ? one *= -1 : 0;
		int a = i % 3 == 2, b = a ? -2 : 1;
		for (int j = 0; j < 4; j++) {	// rotating one face around
			vec3 temp;
...
// format: vec3 position, vec3 colour, vec3 normal
